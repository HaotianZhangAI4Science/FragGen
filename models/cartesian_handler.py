...
class CartesianHandler(nn.Module):
    ...
    def forward(self, node_s, edge_s, edge_index, total_pos, update_node_idx, update_pos=True):
        ...
        # update coords
        if update_pos:
            total_pos = self.update_layer(a_ij, total_pos, edge_index, update_node_idx)
...
class coords_update(nn.Module):
    ...
    def forward(self, a_ij, pos, edge_index, update_node_idx):
        edge_index_mask = torch.isin(edge_index[0], update_node_idx)
        ...
        # delta_x = scatter(delta_x, index=i-i.min(), reduce='sum', dim=0)
        unique_values, _ = torch.unique(i, sorted=True, return_inverse=True)
        reordered_i = torch.searchsorted(unique_values, i)
        delta_x = scatter(delta_x, index=reordered_i, reduce='sum', dim=0)
        pos[update_node_idx,:] += delta_x
        return pos