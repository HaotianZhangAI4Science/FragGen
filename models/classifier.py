...
from models.geom_block import GaussianSmearing, EdgeExpansion, GVLinear, GVPerceptronVN, MessageModule

class FragAtomTypeNet(Module):
    def __init__(self, in_sca, in_vec, hidden_sca, hidden_vec, out_class=125, edge_channels = 64, cutoff = 10.0,):
        super().__init__()
        ...
        self.message_module = MessageModule(in_sca, in_vec, edge_channels, edge_channels, hidden_sca, hidden_vec, cutoff)
        self.classifier = Sequential(
            GVPerceptronVN(hidden_sca, hidden_vec, hidden_sca, hidden_vec),
            GVLinear(hidden_sca, hidden_vec, out_class, 1)
        )

        # provide the information for the following networks
        self.out_hidden_sca = hidden_sca
        self.out_hidden_vec = hidden_vec

    def forward(self, pos_query, pos_compose, node_attr_compose, edge_index_q_cps_knn, edge_index_q_cps_knn_batch):

        vec_ij = pos_query[edge_index_q_cps_knn_batch] - pos_compose[edge_index_q_cps_knn[1]]
        ...
        y = [scatter_add(h[0], edge_index_q_cps_knn_batch, dim=0, dim_size=pos_query.size(0)), # (N_query, F)
                scatter_add(h[1], edge_index_q_cps_knn_batch, dim=0, dim_size=pos_query.size(0))]
        
        y_frag, _ = self.classifier(y) 
        return y_frag
